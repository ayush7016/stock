...
async function fetchDetail(symbol, type) {
  if (symbol && type) {
    changeActiveItem();
    try {
      let interval = type === "INTRADAY" ? "5min" : "daily";
      let fetchData = await fetch(
        `https://www.alphavantage.co/query?function=TIME_SERIES_${type}&symbol=${symbol}&interval=5min&apikey=6SZ4JNZ4YFKUPTA5`
      );
...
;
    ...
    document.querySelector(".option-button.active")
  }
}
...
function showDetails(event) {
  ...
  if (event.getAttribute(`data-${itemID}`) != 0) {
    ...
    detailedModal.className = `${itemID}-detail detailed-modal`;
